...
const MainDash = () => {
    ...
    useEffect(() => {
        ...
        const getShortenedUrls = async () => {
            ...
            try {
                const response = await fetch(`https://day44backend.vercel.app/shorturl/all/${user.id}`, {
                    method: "GET",
                    headers: {
...
const CardComponent = ({id,shortenedUrl,clicks,setAllUrls,allUrls}) => {
    ...
    const handleDelete = async (id) => {
        setClicked(true)
        try {
            const response = await fetch(`https://day44backend.vercel.app/shorturl/delete/${id}`, {
                method: "DELETE",
                headers: {
                    "user-login-token": localStorage.getItem("usertoken")
                }
